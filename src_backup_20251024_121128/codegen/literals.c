...
static int codegen_register_strlit_value_internal(const char* val) {
    ...
    if (new_id == 53) {
        printf("DEBUG: registrando STRLIT_53 en índice %d con valor (len=%zu)\n", _strlit_count-1, strlen(_strlit_values[_strlit_count-1]));
    }
...
static int codegen_find_strlit_internal(const char* val) {
    ...
    if (is_problematic) {
        printf("DEBUG: buscando STRLIT problemático (len=%zu, _strlit_count=%d)\n", strlen(val), _strlit_count);
        printf("DEBUG: bytes de la cadena buscada: ");
        ...
        if (_strlit_count > 52 && _strlit_values[52]) {
            printf("DEBUG: STRLIT_53 (_strlit_values[52]) = '%s' (len=%zu, primeros bytes: ", _strlit_values[52], strlen(_strlit_values[52]));
            for (size_t j = 0; j < strlen(_strlit_values[52]) && j < 30; ++j) printf("%02x ", (unsigned char)_strlit_values[52][j]);
            printf(")\n");
        } else {
            printf("DEBUG: STRLIT_53 no está disponible (_strlit_count=%d)\n", _strlit_count);
        }
    }
    for (int i = 0; i < _strlit_count; ++i) {
        if (_strlit_values[i]) {
            if (is_problematic && strstr(_strlit_values[i], "compuesto")) {
                printf("DEBUG: encontrado STRLIT_%d que contiene 'compuesto' (len=%zu)\n", i+1, strlen(_strlit_values[i]));
                printf("DEBUG: bytes del STRLIT_%d: ", i+1);
            ...
            if (strcmp(_strlit_values[i], val) == 0) {
                if (is_problematic) printf("DEBUG: STRLIT encontrado como STRLIT_%d\n", i+1);
                return i+1;
            }
        }
    }
    if (is_problematic) printf("DEBUG: STRLIT NO encontrado (revisados %d STRLITs)\n", _strlit_count);
    return -1;
}
...
static int codegen_register_numlit_value_internal(const char* sval) {
    ...
    printf("DEBUG: registrando literal '%s' -> valor=%f -> bits=0x%016llx\n", sval, v, bits);
    ...
    printf("DEBUG: literal registrado como NUMLIT_%d\n", _numlit_next_id+1);
...
int codegen_find_numlit(const char* val) { 
    ...
    printf("DEBUG: buscando literal '%s' -> valor=%f -> bits=0x%016llx\n", val, v, bits);
    for (int i=0;i<_numlit_count;++i) {
        printf("DEBUG: comparando con NUMLIT_%d bits=0x%016llx\n", i+1, _numlit_bits[i]);
    ...
    if (strlen(val) > 1 && (val[strlen(val)-1] == 'f' || val[strlen(val)-1] == 'F')) {
        ...
        printf("DEBUG: intentando sin sufijo '%s' -> valor=%f -> bits=0x%016llx\n", val_without_f, v2, bits2);
        for (int i=0;i<_numlit_count;++i) {
            printf("DEBUG: comparando sin sufijo con NUMLIT_%d bits=0x%016llx\n", i+1, _numlit_bits[i]);
...
void codegen_emit_all_numlits(FILE* f) {
    if (!f) return;
    printf("DEBUG: emitiendo %d literales numéricos\n", _numlit_count);
    for (int i=0;i<_numlit_count;++i) {
        unsigned long long bits = _numlit_bits[i];
        printf("DEBUG: emitiendo NUMLIT_%d con bits=0x%016llx\n", i+1, bits);
        fprintf(f, "    .align 8\n");
        fprintf(f, "NUMLIT_%d:\n", i+1);
